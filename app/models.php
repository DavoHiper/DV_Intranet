                ...
                switch($host)
                {
                    ...
                    */
                    case EXTRANET_HOST:
                        ...
                        break;
                    default: 
                        if (self::autenticaAD($this->usuario, $this->senha))
							return '1';
						else
							return '0';						
                        break;
...
//            echo  $sql;
    ...
    public function linkArquivo($CategoriaCodigo,   $ArquivoCodigo, $Extensao)
    {
        ...
        $link = "http://".$_SERVER[HTTP_HOST]."/intranet_41".CAMINHO_ARQUIVOS."CAT".$this->preencheZero("E", 4, $CategoriaCodigo)."/".$this->preencheZero("E", 4, $ArquivoCodigo).".".$Extensao;
    ...
    */    
    ...
    public function retornaHtmlComboTodasLojas($loja)
    {
        $sql    =   "  SELECT T06.T006_codigo   CodigoLoja
                            , T06.T006_nome     NomeLoja
                         FROM T006_loja T06";
        ...
        $htmlComboLojas     =   "<label class='label'>Loja</label>";
        $htmlComboLojas    .=   "<select name='T006_codigo' id='comboLojas' class='comboLojas'>";
        $htmlComboLojas    .=   "<option value=''></option>";                   
    ...
    public function retornaHtmlFuncoesColaboradores($funcao)
    {
        $sql    =   "  SELECT T121.T121_codigo CodigoFuncao
                            , T121.T121_nome   NomeFuncao
                         FROM T121_funcoes_colaboradores T121";

        $dados  =   $this->query($sql);
        
        $htmlComboFuncoes     =   "<label class='label'>Funções</label>";
        $htmlComboFuncoes    .=   "<select name='T121_codigo' id='comboFuncoes' class='comboFuncoes'>";
        $htmlComboFuncoes    .=   "<option value=''></option>";                   
        
        foreach($dados  as  $campos=>$valores)
        {           
            if ($valores['CodigoFuncao'] == $funcao)
                $selected   =   "selected";
            else
                $selected   =   "";
            
            $htmlComboFuncoes .=  "<option value='".$valores['CodigoFuncao']."' $selected>".$this->preencheZero("E", 3, $valores['CodigoFuncao'])."-".$valores['NomeFuncao']."</option>";
        }
        
        $htmlComboFuncoes .=  "</select>";
            
        
        return $htmlComboFuncoes;
    }

    public function retornaHtmlDeptosColaboradores($depto)
    {
        $sql    =   "  SELECT T77.T077_codigo   CodigoDepto
                            , T77.T077_nome     NomeDepto
                         FROM T077_departamento T77";

        $dados  =   $this->query($sql);
        
        $htmlComboDeptos     =   "<label class='label'>Departamentos</label>";
        $htmlComboDeptos    .=   "<select name='T077_codigo' id='comboDeptos' class='comboDeptos'>";
        $htmlComboDeptos    .=   "<option value=''></option>";                   
        
        foreach($dados  as  $campos=>$valores)
        {           
            if ($valores['CodigoDepto'] == $depto)
                $selected   =   "selected";
            else
                $selected   =   "";
            
            $htmlComboDeptos .=  "<option value='".$valores['CodigoDepto']."' $selected>".$this->preencheZero("E", 3, $valores['CodigoDepto'])."-".$valores['NomeDepto']."</option>";
        }
        
        $htmlComboDeptos .=  "</select>";
            
        
        return $htmlComboDeptos;
    }
