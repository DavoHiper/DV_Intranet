...
//            $this->alerts('true', 'Erro!', 'Não foi possível Alterar!');          
    ...
    public function retornaArquivos($codigoArquivo, $codigoAuditoria, $lojaBusca, $departamentoBusca, $dataInicial, $dataFinal)
    { 
        ...
        $sql    =   "  SELECT T93.T093_codigo        CodigoArquivo
         ...
         if(!empty($codigoAuditoria))
            $sql .= " AND T93.T093_codigo = ".$codigoAuditoria;
        if(!empty($lojaBusca))
            $sql .= "   AND  T06.T006_codigo  = ".$lojaBusca;
        if(!empty($departamentoBusca))
            $sql .= "   AND  T20D.T020_departamento  = ".$departamentoBusca;
        if(($dataInicial != "null") && ($dataFinal != "null"))
            $sql .= "  AND T093_dt_inicio  between '".$dataInicial."' and '".$dataFinal."'";
        if(($dataInicial != "null") && ($dataFinal == "null"))
            $sql .= "   AND  T093_dt_inicio  >= '".$dataInicial."' and T093_dt_inicio <= DATE_ADD('$dataInicial', INTERVAL 1 DAY) ";
        if(($dataInicial == "null") && ($dataFinal != "null"))
            $sql .= "   AND T093_dt_inicio  >= '".$dataFinal."' and T093_dt_inicio <= DATE_ADD('$dataFinal', INTERVAL 1 DAY) ";
...
//                $headers   .=   "Cc: web@davo.com.br, marcio@davo.com.br";
                $headers   .=   "Cc: ralfieri@davo.com.br, rsnascim@davo.com.br, marcio@davo.com.br";
