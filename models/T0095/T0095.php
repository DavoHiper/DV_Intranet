...
//            $this->alerts('true', 'Erro!', 'Não foi possível Incluir!');
    ...
    public function retornaRupturasLoja($codigoArquivo)
    {
        $sql    =   "   SELECT T94.T093_codigo                              Auditoria
                         ...
                         WHERE T94.T093_codigo = $codigoArquivo
                           AND T94.T093_tipo = 'C'
                           AND T94.T094_linha= 'S'
                           AND T94.T094_codigo_rms NOT IN (
                                                                SELECT T94.T094_codigo_rms
                                                                  FROM T094_auditoria_detalhes T94 
                                                                 WHERE T093_codigo = $codigoArquivo
                                                                   AND T093_tipo = 'I' 
                                                             )
                           AND T94.T094_estoque_rms >   0
             GROUP BY T94.T094_codigo_rms
             ORDER BY 7 DESC";
  ...
  //      echo $sql."<br>";
    ...
    public function retornaDadosGraficoMeta($data, $meta, $loja) {
        
        $sql    =   "SELECT count(*)                                Qtd
                        ,   date_format(T093_dt_inicio,'%d/%m') Data 
                        FROM T093_auditoria
                       WHERE T093_tipo = 'I'";
        
        if(!empty($data)){
            $sql .= " AND T093_dt_inicio >= '$data'";
            $sql .= " AND T093_dt_inicio < date_add('2013-07-20', INTERVAL 1 DAY)";
        }
        if(!empty($loja)){
        " AND T006_codigo = ".$loja;}
        
        $sql .= " GROUP BY date_format(T093_dt_inicio,'%d/%m')";
