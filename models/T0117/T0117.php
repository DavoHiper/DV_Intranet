...
//       
    ...
    public function retornaRM($titulo, $descricao, $solicitante, $codRM, $user, $status)
    {        
        ...
        if(!empty($status))
            $sql    .=  " AND T113.T113_status          =   '$status'";
...
//echo $sql;
    ...
    public function mostraBotao($perfil, $user, $status, $codRM) {
        ...
        } elseif(($perfil == 59 ) && (($status == 3)||($status  ==  4)||($status == 5)||($status == 6))) {
    ...
    public function enviaEmailExec($user, $codRM, $tipo, $titulo){
        ...
        $headers .= "Cc: rsnascim@davo.com.br, ralfieri@davo.com.br";
...
$sql    =   "SELECT T04.T004_login      Login
    ...
    public function enviaEmailSolicitante($codRM, $titulo, $status){
$sql    =   "SELECT T04.T004_login      Login
                  , T04.T004_email      Email
                  , T04.T004_nome       Nome
                  , T113.T113_status    Status
                  , T113.T113_titulo    Titulo
               FROM T004_usuario T04
               JOIN T113_requisicao_mudanca T113
                 ON (T113.T004_solicitante = T04.T004_login OR T113.T004_responsavel = T04.T004_login)
              WHERE T113.T113_codigo =  ".$codRM;       
        ...
        $to         = $valEmail["Email"]; 
        $from       = "web@davo.com.br"; 
        $subject    = "[Intranet] - Aviso de Status RM ".$codRM." - ".$valEmail["Titulo"];
        
        $html   =   $valEmail["Nome"].'<br>';
        $html   .=   'A RM Nº '. $codRM.' passou para o STATUS: '.$n_status."<br>";
        $html   .=   'Requisição Nº '. $codRM.' - '.$valEmail["Titulo"];
