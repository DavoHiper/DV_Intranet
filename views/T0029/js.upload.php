...
//print_r($_FILES);
 ...
 $depto = $objUpload->retornaDepartamentoUser($_SESSION['user']);
 
 $dp = 0;
 foreach ($depto as $cpsDepto => $vlrDepto) {
    $dp++;
...
if(($_POST['T056_codigo'] == 22 || $_POST['T056_codigo'] == 3 || $_POST['T056_codigo'] == 29) && ($dp <= 0)) {
    
    echo "<script>alert('Por favor encaminhar arquivo para a área GSP.".$dp."');</script>";
        echo "<script>window.location = '?router=T0029/novo';</script>";   
...
if (empty($_POST["T055_codigo"]))
   {//valida o tipo, se for vazio, é um upload não existente, se for igual a 1, o usuário esta
    ...
    if (!empty($_FILES["P0029_arquivo"]["name"]))
       {
                   ...
                   { //move o arquivo que esta sendo upado no sistema do temporário para o path de destino               
                    if (rename($path_novo, $path_novo_rename))
                       {//renomeia o arquivo recem movido do temporário para o Código do arquivo no banco 
                        ...
                        if($_SESSION["user"] == $_POST['T004_owner'] && $_POST["T055_publico"] != 1){
