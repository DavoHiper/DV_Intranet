    ...
    <div id="ferr-conteudo">
        <span class="ferr-cont-menu">
            <ul>
                ...
                <li><a href="?router=T0071/home" >Relatório por Loja</a></li>
                <li><a href="?router=T0071/categoria" class="active">Relatório por Categoria</a></li>
            </ul>
        </span>
    </div>
</div>

<form action="?router=T0071/categoria" method="post">
...
$categorias      =   $obj->retornaCategorias(); 
...
<div id="conteudo">
    <span class="lista_itens">
        <table>
            <tr>
                ...
                foreach($totalNotas as $campos=>$valores)
                {
                    $total = $valores['Total'];
                ?>
                    <td style="font-size: 15px; font-weight: bold;"><?php echo "Total: ".$obj->formataMoeda($valores['Total']); ?></td>                
                <?php
                }
                ?>
            </tr>
        ...
        <?php
            ...
            foreach($categorias as $campos=>$valores)
            {   // Inicio da listagem das lojas 
                // Busca as notas fiscais passando alguns parametros
                $listaNotas =   $obj->retornaNotasFornecedorCategoria($codigo, $dt_inicial, $dt_final,$valores['CodigoCategoria']);
                ...
                {   // Inicio da listagem das notas 
                    ...
                    if($i==0)
                        echo "<tr style='font-weight: bolder;'><td colspan='4'>".$valores['CodigoCategoria']." - ".$valores['NomeCategoria']."</td></tr>";
                    ...
                    <tr style="background:<?php echo $cor; ?>;">
                        ...
                        <td align="right"><?php echo $valor_bruto   = substr($obj->formataMoeda($valores2['ValorBruto']), 2);  ?></td>
                ...
                if ($totalLoja != 0)
                    echo "<tr style='font-weight: bolder; background: #F4F4F4;'><td colspan='2'>Total da Loja: </td><td align='right'>".substr($obj->formataMoeda($totalLoja),2)."</td></tr>";      
                 
                // Iguala o total da loja para iniciar a contagem com outra loja no FOR
                $totalLoja = 0;
                
            }   // Final da listagem das categorias
        ?>
            </tbody>
        </table>
    </span>
